    ...
    public void consume(Message message) {
        try {
            ...
            if (messageId != null && !idempotencyService.checkAndSave(messageId)) {
                log.info("Duplicate in-app message ignored: {}", messageId);
                return;
            }

            Map<String, Object> payload = objectMapper.readValue(message.getBody(), new TypeReference<Map<String, Object>>() {});
            log.info("Received in-app event: {}", payload);

            NotificationChannelStrategy strategy = notificationFactory.getStrategy("IN_APP");
            strategy.send(payload);

        } catch (Exception e) {
            log.error("Failed to process in-app message", e);
