...
public class WishlistEventProducer {
    ...
    public void publishWishlistAddedEvent(WishlistEvent event) {
        ...
        rabbitTemplate.convertAndSend("social.events.exchange", "social.wishlist.added", event, message -> {
    ...
    public void publishWishlistRemovedEvent(WishlistEvent event) {
        ...
        rabbitTemplate.convertAndSend("social.events.exchange", "social.wishlist.removed", event, message -> {
            message.getMessageProperties().setMessageId(UUID.randomUUID().toString());
            return message;
        });
    }
