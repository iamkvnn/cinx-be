...
public class VoucherController {
    ...
    @GetMapping
    public ResponseEntity<PaginatedApiResponse<?>> getVouchers(@Valid @ModelAttribute PaginatedApiQuery query) {
        return ResponseEntity.ok(PaginationWrapper.wrap(voucherService.getVouchers(query)));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<?>> getVoucherById(@PathVariable String id) {
